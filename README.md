...
SPEC.md itself was generated by prompting Claude with:

> Scope/make a plan for a small Android app. Your output will be used as input to a coding agent
>
> Here's the user story: [Jeff's post pasted above]
>
> I'm thinking on an app that fetch/sync the phone calendar data (selected by the user). Display the upcoming meetings. As well as trigger some alarm + notification at T - n (n is 1min in the example). Make sure it works even if the app is in the background.
>
> Please keep it simple and straightforward. Do not overengineer it - it's an MVP
